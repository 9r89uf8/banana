...
export const imageLibraryService = {
  ...
  async saveImage(imageData) {
    try {
      ...
      const docRef = doc(db, IMAGE_LIBRARY_COLLECTION, imageData.id);
      await setDoc(docRef, libraryImageData);
      return imageData.id;
    } catch (error) {
  ...
  async getImages(userId = null, maxResults = 100, tags = null) {
    try {
      ...
      if (tags && tags.length > 0) {
        // Filter by tags
        q = query(
          collection(db, IMAGE_LIBRARY_COLLECTION),
