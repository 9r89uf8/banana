...
import { useGenerationQueueContext } from '@/app/contexts/GenerationQueueContext';

export default function QueueItem({ generation, onView }) {
  const { retryGeneration, runAgain, removeFromQueue } = useGenerationQueueContext();
  ...
  const downloadOriginal = async () => {
    ...
    if (generation.result?.imageUrl) {
      ...
      link.download = generation.result.fileName?.split('/').pop() || 'composition.png';
      link.click();
    }
  };
      ...
      <div className="aspect-square bg-gray-100 flex items-center justify-center p-2">
        {generation.status === 'completed' && generation.result ? (
          ...
          <div className="grid grid-cols-2 gap-1 w-full h-full">
            <img
              src={generation.thumbnails.main}
              alt="Main image"
              className="w-full h-full object-cover rounded"
            />
            <img
              src={generation.thumbnails.person}
              alt="Person image"
              className="w-full h-full object-cover rounded"
            />
          </div>
